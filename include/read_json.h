  ...
  std::vector<std::shared_ptr<Light> > & lights)
{
    ...
    [&parse_Vector3d](const json & j, Camera & camera)
  {
	...
	camera.focal_plane = j["focal_plane"].get<double>();
  };
    ...
    std::unordered_map<std::string,std::shared_ptr<Material> > & materials)
  {
    ...
    for(const json & jmat : j)
    {
	  ...
	  if(jmat.count("ior")){	 
	  	...
	  	if (material->ior < 1.0) {material->ior = 1.0;}	  
	  }

	  if(jmat.count("type")){
	  	if (jmat["type"] == "glossy"){
			material->is_glossy = true;
		}	
	  }
    ...
    std::vector<std::shared_ptr<Light> > & lights)
  {
    ...
    for(const json & jlight : j)
    {
      ...
      }else if(jlight["type"] == "point")
      {
        ...
        light->I = parse_Vector3d(jlight["color"]);
        lights.push_back(light);
      }else if(jlight["type"] == "spot") //mine
      {
        std::shared_ptr<SpotLight> light(new SpotLight());
        light->p = parse_Vector3d(jlight["position"]);
        light->I = parse_Vector3d(jlight["color"]);
		light->cutoff_angle = jlight["angle"].get<double>(); 
        light->d = parse_Vector3d(jlight["direction"]).normalized();
        lights.push_back(light);
      }
    }
